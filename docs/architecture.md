...
- `src/core/storage/normalized-page-session-store.ts` is the canonical NormalizedPage session authority for page-aware modules. It owns exactly one active normalized document/session per mode (`pages`, `selectedPageIndex`, `sourceName`, `documentFingerprint`, `sessionId`) and exposes thin async mutators (`setNormalizedDocument`, `selectPage`, `clearSession`). The store is partitioned by `NormalizedPageSessionMode` (`'config' | 'run'`); `getNormalizedPageSessionStore('config')` and `getNormalizedPageSessionStore('run')` return module-level singletons keyed by mode, so Config Capture and Run Mode share one store module and one fingerprint formula but never clobber each other's pages while both screens are mounted. The single fingerprint formula is exported as `buildDocumentFingerprint(sourceName, pages)` and is the only correct way for a feature to derive a `documentFingerprint`.
...
- Runtime page state is owned by the canonical `NormalizedPageSessionStore` under the `'run'` partition. Run Mode subscribes via `useSyncExternalStore`; it does not maintain a local `runtimePages` array, a local selected-page index, or a hand-rolled `documentFingerprint`. Page selection in the UI calls `runtimeSessionStore.selectPage`, normalization writes via `runtimeSessionStore.setNormalizedDocument`, and parse failures call `runtimeSessionStore.clearSession`. The session store derives `documentFingerprint` via the shared `buildDocumentFingerprint` helper, so Config and Run produce identical fingerprints for identical inputs.
- Both Config Capture and Run Mode build the structural overlay status line through the shared `buildStructuralStatusText` helper in `src/core/page-surface/ui/structural-status-text.ts`. The helper is pure and accepts an optional `transformationModel` so Run Mode can surface alignment confidence in the same line shape Config uses for CV adapter + page CV mode + OpenCV runtime status.
